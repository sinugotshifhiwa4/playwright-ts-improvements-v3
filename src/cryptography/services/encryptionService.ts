...
export default class EncryptionService {
   ...
   *   operations.
   */
  public static async encrypt(value: string, secretKey: string): Promise<EncryptionResult> {
    try {
   ...
   */
  private static parseEncryptedData(encryptedData: string): EncryptionResult {
    try {
      if (!encryptedData) {
        ErrorHandler.logAndThrow('Encrypted data is required.', 'parseEncryptedData');
      }

      const parsedData = JSON.parse(encryptedData) as EncryptionResult;
   ...
   */
  private static validateParsedData(params: EncryptionResult): void {
    try {
